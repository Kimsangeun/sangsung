...
public class ResQnAReplyReg implements Action {
	...
	public ActionData exec(HttpServletRequest request, HttpServletResponse responce) {
		...
		String url = "ResQnADetail?id="+vo.getId()+"&seq=0";
		...
		if(vo.getUpfile().equals("") || imgChk(ext)) {
			dao.resReplyInsert(vo);
			msg = "답글이 등록되었습니다.";
		
		}
		
		request.setAttribute("replydata", dao.resReplyList(id));
		request.setAttribute("data", dao.resDetail(id, 0));
	...
	public String fileUpload(HttpServletRequest request) {
		...
		try {
			...
			if(pp.getContentType()!=null) {
				...
				System.out.println(imgChk(fileName.substring(fileName.lastIndexOf(".")+1).toLowerCase()));
